  ...
  <div class="kt-portlet kt-portlet--height-fluid pt-2">
    ...
    >
      <div class="kt-portlet__head-label mb-2">
        ...
        <h3 class="kt-portlet__head-title">Phone Shops</h3>
      </div>
    </div>
    <div class="kt-portlet__body">
      ...
      >
        <template v-slot:item.start_date="{ item }">
          <span>{{ item.start_date | displayDateTime }}</span>
        </template>
  ...
  methods: {
    ...
    setHeaders() {
      ...
      if (
        ...
        isAdmin ||
        isSecretShopper ||
        (isSalesperson && this.$auth.user().specific_dealer_id)
      ) {
    ...
    searchPhoneShop: _debounce(function () {
      this.options.page = 1;
      this.fetchData();
    }, 500),
    updatePage(page) {
    ...
    fetchData() {
      ...
      this.$http
        .get('/clients/secret-shops/phone-shops', {
          params: {
